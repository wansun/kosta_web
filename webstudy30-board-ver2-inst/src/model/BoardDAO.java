...
public class BoardDAO {
	 ...
	 * SELECT b.no,b.title,b.hits,to_char(time_posted,'YYYY.MM.DD') as time_posted,m.id,m.name 
		...
		매개변수로 PagingBean을 입력받아서
		SQL의 between의 시작번호와 마지막번호를 할당하여 조회
	 * @param pageNo
	 * @return
	 * @throws SQLException
	 */
	public ArrayList<PostVO> getPostingList() throws SQLException{
		...
		try{
			...
			sql.append(" select b.no, b.title, b.timeposted, b.hits, b.id, m.name");
			sql.append(" from (");
			sql.append(" select row_number() over(order by no desc) rnum, no, title, id, hits,");
			sql.append(" to_char(time_posted,'yyyy.mm.dd') timeposted");
